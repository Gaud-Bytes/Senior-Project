...
class Ship:
        ...
        if(isinstance(endSpace, tuple) and len(endSpace) == 2):
            if(isinstance(endSpace[0], int) and isinstance(endSpace[1]), int):
                if(endSpace[0] >= 0 and endSpace[1] >= 0):
                    if(((endSpace[0] == self._startSpace[0] + self._shipSize - 1) or (endSpace[0] == self._startSpace[0] - self._shipSize - 1)) and (self._startSpace[1] == self._endSpace[1])):
                        self._endSpace = endSpace
                        self.__setSquareCoords()
                        return True
                    elif(((endSpace[1] == self._startSpace[1] + self._shipSize - 1 ) or (endSpace[1] == self._startSpace[0] - self._shipSize - 1)) and (self._startSpace[0] == self.endSpace[0])):
                        self._endSpace = endSpace
                        self.__setSquareCoords()
                        return True
                    else:
    ...
    def __setSquareCoords(self):
        for x in range(self._startSpace[0], self._endSpace[0]):
            for y in range(self._startSpace[1], self._endSpace[1]):
                self._shipSpaces.append((x, y))
